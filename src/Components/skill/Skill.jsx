    ...
    return (
        <section id='skills' className='text-gray-900 bg-[#FAEDCD] body-font'>
            <div className='container px-5 py-11 mx-auto'>
                <div data-aos='fade-up' data-aos-delay='300' className='flex flex-col text-center w-full mb-10'>
                    <h1 className='text-3xl font-semibold text-[#432818] mb-4'>My Skills</h1>
                    <p className='text-base text-[#432818] leading-relaxed mx-auto max-w-3xl'>
                        I have experience with front-end and back-end technologies, including JavaScript frameworks, databases, and mobile development.
                        My expertise spans full-stack development, Android applications, and cloud-based solutions.
                    </p>
                </div>

                {/* Skills Grid */}
                <div data-aos='fade-up' data-aos-delay='400' className='flex flex-wrap justify-center -m-4'>
                    {skills.map((skill, index) => (
                        <div key={index} className='p-4 md:w-1/3 sm:w-1/2 w-full'>
                            <div className='bg-white shadow-md rounded-lg p-6 flex justify-between items-center'>
                                ...
                                <div className='flex'>
                                    {[...Array(5)].map((_, i) => (
                                        i < skill.rating ? <FaStar key={i} className='text-yellow-500' /> : <FaRegStar key={i} className='text-gray-400' />
                                    ))}
                                </div>
                            </div>
                        </div>
